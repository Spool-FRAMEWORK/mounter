...
class ReactiveMounterAssembler {
    ...
    static <I, O> Mounter assemble(ReactiveMounterBuilder<I, O> b) {
        ...
        SequentialPartitionDispatcher seq = new SequentialPartitionDispatcher();
        Pipeline<PipelineContext, PipelineContext> pipeline = Pipeline.<PipelineContext>start()
                .add(new DiscoverPartitionsStep(scope -> List.of(PartitionInfo.of(scope))))
                .add(new ResolveScalingPlanStep(partitions -> ScalingPlan.sequential()))
                .add(new BuildMountUnitsStep(null))
                .add(new DispatchUnitsStep(seq, seq, unitHandler));
